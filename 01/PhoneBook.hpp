...
/*   Updated: 2022/01/29 21:42:29 by csouza-f         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void PhoneBook::search_contact()
{
    ...
    std::cout << std::endl;
    std::string input;
    while (std::cout << "> SEARCH: Enter index: " && std::getline(std::cin, input)) {
        if (input == "EXIT" || input == "exit" || input == "quit" || input == "e" || input == "q")
            break ;
        if (input == "add" || input == "search" || input == "ADD" || input == "SEARCH") {
            std::cout << "Error: Type EXIT to go back to Phonebook." << std::endl;
            continue ;
        }
        ...
        std::string contact_keys[CONTACT_INPUTS] = {"First name", "Last name", "Nickname", "Phone number", "Darkest secret"};
        if (contacts[contact_index].first_name.empty()) {
            std::cout << "Error: Index's contact is empty." << std::endl;
            continue ;
        }
        std::string contact_values[CONTACT_INPUTS] = {contacts[contact_index].first_name,
        ...
        for (unsigned int i = 0; i < CONTACT_INPUTS; i++) {
            std::cout << contact_keys[i] + ": " << contact_values[i] << std::endl; 
        }
    }
}
...
void PhoneBook::add_contact()
{
    ...
    std::string questions[CONTACT_INPUTS] = {"First name", "Last name", "Nickname", "Phone number", "Darkest secret"};
    ...
    std::cout << "Success: Contact `" + contacts[i].first_name + "` added." << std::endl;
    i++;
}
